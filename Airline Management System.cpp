...
//Pilot Eligibility
bool isSafe2(int v, vector<vector<int>> graph, vector<int> color, int c, int n) {
    for (int i = 0; i < n; i++) {
...
bool graphColoring(vector<vector<int>> graph, int m, int n) {
    ...
    cout << "Solution exists and the colors of the vertices are:\n";
    for (int i = 0; i < n; i++)
    cout << "Airplane " << char(64+i+1) << " can be piloted by Pilot " << color[i] << endl;
    return true;
}
//Avoid Bad Weather
...
class about
{
	public:
		void Aboutus()
		{
			cout<<"Introducing Vertigo Airlines - your one-stop destination for all your air travel needs in India!\nWith a wide range of local and international flights,\nwe offer unparalleled comfort and convenience for both business and leisure travelers.\nOur fleet of modern aircraft, coupled with our experienced and friendly crew,\nensures that your travel experience is nothing short of extraordinary.\nWhether you're looking for a quick domestic flight or an exciting international adventure, we've got you covered.\nSo why wait? Book your next flight with Vertigo Airlines and get ready to soar to new heights!\n\n\n"<<endl<<endl;
...
int main()
{
	...
	main:
	system("COLOR 6f");
	cout<<"\n\t\t\t\tAlgorithms and Problem Solving LAB MINI PROJECT\n";
	cout<<"\t\t\t\t\tTopic: AIRLINE MANAGEMENT SYSTEM\n";
	...
	cout<<"\n\n";
	cout<<"\n\t\t _    __          __  _                ___    _      ___                ";
	cout<<"\n\t\t| |  / /__  _____/ /_(_)___ _____     /   |  (_)____/ (_)___  ___  _____";
	cout<<"\n\t\t| | / / _ \\/ ___/ __/ / __ `/ __ \\   / /| | / / ___/ / / __ \\/ _ \\/ ___/";
	cout<<"\n\t\t| |/ /  __/ /  / /_/ / /_/ / /_/ /  / ___ |/ / /  / / / / / /  __(__  ) ";
	cout<<"\n\t\t|___/\\___/_/   \\__/_/\\__, /\\____/  /_/  |_/_/_/  /_/_/_/ /_/\\___/____/  ";
	cout<<"\n\t\t                    /____/              Fly with us around the world!                                 ";
	cout<<"\n";

	cout<<endl<<"\n\n\nWelcome to VERTIGO Airlines"<<endl;
	...
	if(choice==49)
	{
		...
		if(pass=="admin")
		{
			...
			<<endl<<"Press 2 to Check Pilot Eligibility"
			<<endl<<"Press 3 to Check for Bad Weather Conditions"
			<<endl<<"Press 4 to Check for Shortest Path"
			...
			case 48:	//choice = 0
			{	//Exit
				cout<<"Thank You for using VERTIGO Airlines\n";
				exit(0);
			}
			...
			case 49:
			{
				cout << "Enter the Number of Vertigo Airplanes present: ";
				cin >> n;
			...
			case 50:
			{
				int num_planes = 5, num_pilots = 3;
				cout<<"Currently there are 5 Airplanes and 3 Pilots.\nPilot 1 ----- can Pilot Airplane A,D and E only. \nPilot 2 ----- can Pilot Airplane A,B and E only. \nPilot 3 ----- can Pilot Airplane B and C only.\n"<<endl;
				vector<vector<int>> graph(num_planes, vector<int>(num_pilots, 0));

				graph[0][0] = 1; // Plane A can be Piloted by Pilot 1 and Pilot 2
				graph[0][1] = 1;
				graph[0][2] = 0;

				graph[1][0] = 1; // Plane B can be Piloted by Pilot 1 and Pilot 3
				graph[1][1] = 0;
				graph[1][2] = 1;

				graph[2][1] = 1; // Plane C can be Piloted by Pilot 1 and Pilot 2
				graph[2][0] = 1;
				graph[2][2] = 0;

				graph[3][2] = 1; // Plane D can be Piloted by Pilot 3
				graph[3][0] = 0;
				graph[3][1] = 0;

				graph[4][1] = 1; // Plane E can be Piloted by Pilot 2
				graph[4][0] = 0;
				graph[4][2] = 0;


				bool flag = graphColoring(graph, num_pilots, num_planes);
				cout<<"\n\nPress Enter to go back to Admin Menu...."<<getch();
				system("cls");
				goto admin;
				break;
